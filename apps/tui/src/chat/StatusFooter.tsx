...
export function TeamStatusRow(props: { model: TeamLiveView }) {
...
export function StatusFooter(props: {
  ...
  return (
    <box width="100%" height={1} flexDirection="row" paddingX={2}>
      ...
      <box flexGrow={1} />
      <text fg="#6e7681" wrapMode="none" truncate>{`${props.options.modeName} | ${props.options.modelName} | ${props.options.providerName} | ${actionHint} | tab agents | ctrl+p commands`}</text>
    </box>
  );
}
