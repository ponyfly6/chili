...
export function RunsPanel(props: PanelProps) {
  ...
  return (
    <box flexGrow={1} minWidth={24} height="100%" flexDirection="column" border borderStyle="single" borderColor={props.focused ? "#88c0d0" : "#3b4252"} paddingX={1}>
      <text fg="#f8f8f2" truncate wrapMode="none">{focusLabel(`Runs / Health${runWindow.label}`, props.focused)}</text>
      {health ? (
        <text fg="#d8dee9" truncate wrapMode="none">
        ...
        <text fg="#8f9baa" truncate wrapMode="none">{"health: unknown"}</text>
      )}
      {runs.length === 0 ? (
        <text fg="#8f9baa" truncate wrapMode="none">{"  run: none"}</text>
      ) : (
        runWindow.rows.map(({ item: run, index }) => (
          <text key={run.id} fg={index === props.selectedIndex ? "#f8f8f2" : "#d8dee9"} truncate wrapMode="none">
      ...
      {tools.slice(0, VISIBLE_LIMITS.activeTools).map((tool) => (
        <text key={tool.id} fg={tool.waitingForApproval ? "#ffd166" : "#a3be8c"} truncate wrapMode="none">
